              ...
              <div class="card-body">
                <form method="POST" id="roleRequest">
                  ...
                  <div class="mb-3 col-md-4">
                    ...
                    <input type="text" class="form-control" id="name" name="name" placeholder="enter role type" value="{{ $data->name }}" onkeyup="$('#slug').val(this.value.toLowerCase())" required>
                  </div>
                  <div class="mb-3 col-md-4 d-none">
                    <label class="form-label" for="basic-default-fullname">Role Slug</label>
                    <input type="text" class="form-control" id="slug" name="slug" placeholder="enter role slug" value="{{ $data->slug }}" required>
                  </div>
                  <div class="mb-3 col-md-4">
                    <label class="form-label" for="basic-default-company">Permission</label>
                    <select class="form-control" name="permission_type" id="permission_type"  onchange="permissionCheck(this.value)" required>
                      <option value="">Select</option>
                      <option value="1" {{ $data->all=="1"?"selected":"" }}>All</option>
                      <option value="0" {{ $data->all=="0"?"selected":"" }}>Custom</option>
                    </select>
                  </div>
                  <div class="mb-3 col-md-4">
                    ...
                    <table class="table table-responsive datatable" style="border: 1px solid lightgray;">
                        ...
                        <tbody>
                            ...
                            @foreach ($activePermission as $permission)
                                @if (in_array($permission->permission_id,$userPermission))
                                    <tr for="select_all">
                  ...
                  <div class="mb-3 d-flex justify-content-center mt-5">
                   ...
                   <button type="submit" id="submit" onclick="requestSend($('#roleRequest'),'{{route('admin.role.update')}}')" name="submit" class="btn btn-primary waves-effect waves-light">Submit</button>
                  </div>
                  </div>
                </form>
              </div>
